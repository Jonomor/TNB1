...
async function run() {
  try {
   ...
   // Create new post entry
    const existingPost = (function() {
      const idx = posts.findIndex(function(p) { return p.id === 'post-' + today; });
    ...
    const newPost = {
      ...
      category: 'Market Analysis',
      image: existingPost ? existingPost.image : '',
      tags: ['XRP', 'XRPL', topicAngle.replace(/_/g, ' '), 'Infrastructure'],
      author: 'K. Morgan',
    ...
    const existingIndex = posts.findIndex(function(p) { return p.id === newPost.id; });
    if (existingIndex >= 0) {
      var preserved = posts[existingIndex];
      Object.keys(newPost).forEach(function(key) { preserved[key] = newPost[key]; });
      posts[existingIndex] = preserved;
